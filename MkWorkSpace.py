...
----------------------------------------------------------------------------------------------------------------------------------
shapes data_obs          REG  WS w:data_obs_MPE_REG
shapes PROC_MASS         REG  WS w:PROC_MASS_MPE_REG w:PROC_MASS_MPE_REG_$SYSTEMATIC
shapes TTJets            REG  WS w:MTP_regionREG_TTJets w:MTP_regionREG_TTJets_$SYSTEMATIC
shapes Other             REG  WS w:MTP_regionREG_Other w:MTP_regionREG_Other_$SYSTEMATIC
shapes QCD               REG  WS w:pdf_regionREG_QCD w:pdf_regionREG_QCD_$SYSTEMATIC
----------------------------------------------------------------------------------------------------------------------------------
bin                REG      
observation        -1.0    
----------------------------------------------------------------------------------------------------------------------------------
bin                REG                REG              REG            REG           REG          REG
process            PROCMASS_WIDTH      TTJets2017     WJets2017    ZJets2017      VV2017        DY2017
process            0                   1                 2               3            4           5
rate               -1                  -1                -1              -1          -1          -1
------------------------------------------------------------------------------------
lumi_had           lnN    1.023        1.023             1.023           1.023      1.023       1.023
    ...
    for proc, regs in procs.items():
        for reg in regs:
            ...
            if re.match('SMS', proc) != None:
                ...
                print('Making datacard proc {0} mass {1} channel {2} region {3}'.format(proc.split('_')[0], mass, reg))
    ...
    for proc in sigs: 
        ...
        for stop in stop_masses:
            ...
            for lsp in lsp_masses: 
                cards = ' '
                for regs in regs:
                    cards = cards + '{1}_{2}=datacard_{0}_{1}_{2}.txt '.format(proc, str(stop)+'_'+lsp, reg)
                ...
                cmd_template = re.sub('MASS', str(stop)+'_'+str(lsp), cmd_template)
                ...
                os.chdir(pwd) 
                os.system('text2workspace.py --channel-masks {0}/datacard_{1}_{2}_{3}.txt'.format(carddir, proc, stop+'_'+str(lsp), reg))
...
combine -M AsymptoticLimits -s 123456 -m STOP -d ../../CARDDIR/datacard_PROC_STOP_LSP.root --run expected -v 5 --rMin -0.1 --rMax 5 >& AsymptoticLimits_PROC_STOP_LSP.log
mv higgsCombineTest.AsymptoticLimits.mHSTOP.123456.root higgsCombineTest.AsymptoticLimits.PROC_STOP_LSP.123456.root
