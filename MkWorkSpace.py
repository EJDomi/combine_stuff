...
rate                       -1                 -1            -1           -1
-------------------------------------------------------------------------------
lumi_had               lnN     1.025              0.975/1.025   1.025/0.975  1.025/0.975 
...
datacard_regionsAC_rate='''
----------------------------------------------------------------------------------------------------------------------------------
QCD_CH_RREG_norm rateParam RREG QCD 1
----------------------------------------------------------------------------------------------------------------------------------
'''

datacard_regionsB_rate=''' 
----------------------------------------------------------------------------------------------------------------------------------
QCD_CH_RREG_norm rateParam RREG QCD 1
----------------------------------------------------------------------------------------------------------------------------------
'''

datacard_regionD_rate='''
----------------------------------------------------------------------------------------------------------------------------------
QCD_CH_RD_norm    rateParam RD QCD       (@0*@1*@3)/@2 QCD_CH_RB_norm,QCD_CH_RC_norm,QCD_CH_RA_norm,ABCDrate
----------------------------------------------------------------------------------------------------------------------------------
'''

datacard_toy_regionD_rate='''ABCDrate     lnN     -                   -                 -               1.6
----------------------------------------------------------------------------------------------------------------------------------
...
def GetProcs():
  ...
  sigs['TbtH']['00p']        = [700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1700, 1800, 2000, 2200, 2400, 2600]
  sigs['TbtZ']['00p']        = [700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1700, 1800]
  sigs['TttH']['00p']        = sigs['TbtH']['00p']
  sigs['TttZ']['00p']        = sigs['TbtZ']['00p']
  sigs['TbtHtZ']['00p']      = [700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1700, 1800]#sigs['TbtZ']['00p']
  sigs['TttHtZ']['00p']      = [700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1700, 1800]#sigs['TbtZ']['00p']

  sigs['TbtH']['10p']     = [700, 800, 1000, 1200, 1400, 1600, 1800, 2000, 2200, 2400, 2600]
  sigs['TbtZ']['10p']     = [700, 800, 1000, 1200, 1400, 1600, 1800, 2000]
  sigs['TttH']['10p']     = sigs['TbtH']['10p']
  sigs['TttZ']['10p']     = sigs['TbtZ']['10p']
  sigs['TbtHtZ']['10p']   = [700, 800, 1000, 1200, 1400, 1600, 1800, 2000]#sigs['TbtZ']['10p']
  sigs['TttHtZ']['10p']   = [700, 800, 1000, 1200, 1400, 1600, 1800, 2000]#sigs['TbtZ']['10p']
  ...
  sigs['TbtHtZ']['20p']   = [800, 1200, 1400, 1600, 1800, 2000]#sigs['TbtZ']['20p']
  ...
  sigs['TbtHtZ']['30p']   = [800, 1000, 1200, 1400, 1600, 1800, 2000]#sigs['TbtZ']['30p']
  sigs['TttHtZ']['30p']   = [800, 1000, 1200, 1400, 1600, 1800]
  #sigs['TttHtZ']['30p']   = [1200]
  ...
  for p, regs in procs.items():
    for r in regs:
      ...
      for s in systs:
        ...
        if 'JER' in s or 'JES' in s: 
          hsystup = fin.Get("MTP_region{0}_{1}_{2}Up".format(r, p, s)).Clone("MTP_region{0}_{1}_{2}Up".format(r, p, s+'_had'))
          hsystdown = fin.Get("MTP_region{0}_{1}_{2}Down".format(r, p, s)).Clone("MTP_region{0}_{1}_{2}Down".format(r, p, s+'_had'))
        ...
        if 'JER' in s or 'JES' in s: 
          roohistup = ROOT.RooDataHist("MTP_region{0}_{1}_{2}Up".format(r, p.replace('estQCD', 'QCD'), s+'_had'), "MTP_region{0}_{1}_{2}Up".format(r, p.replace('estQCD', 'QCD'), s+'_had'), l_mass, hsystup)
          roohistdown = ROOT.RooDataHist("MTP_region{0}_{1}_{2}Down".format(r, p.replace('estQCD', 'QCD'), s+'_had'), "MTP_region{0}_{1}_{2}Down".format(r, p.replace('estQCD', 'QCD'), s+'_had'), l_mass, hsystdown)
        else:
  ...
  for p in sigs: 
    ...
    for w in widths:
      ...
      for m in masses: 
        ...
        os.chdir(pwd) 
        os.system('text2workspace.py --channel-masks {0}/datacard_{1}_{2}_{3}.txt'.format(carddir, p, m, w))
        ...
        os.chdir(pwd) 
        os.system('text2workspace.py --channel-masks {0}/datacard_{1}_{2}_{3}_toy.txt'.format(carddir, p, m, w))
        submitAsymptotic(p, m, w, carddir)

def CombineAllCards(carddir, boostcarddir, resolvecarddir, lepcarddir):
  print("Combining cards")

  chs = ['boosted', 'resolved', 'leptonic']
  sigs = {}
  sigs['boosted'] = {}
  sigs['resolved'] = {}
  sigs['leptonic'] = {}
  ...
  sigs['resolved']['TbtH'] = {}
  sigs['leptonic']['TbtH'] = {}
  sigs['boosted']['TbtH']['00p'] = [800,900,1000,1100,1200,1300,1400,1500,1700,1800]
  sigs['leptonic']['TbtH']['00p'] = [700,800,900,1000,1100,1200,1300,1400,1500,1700,1800]
  sigs['resolved']['TbtH']['00p'] = [700,800,900,1000,1100,1200] 
    ...
    for w in widths:
      ...
      for m in masses: 
        ...
        for ch in chs:
            #REMif not w: w = '00p'
            if 'boost' in ch:
              cards = cards + '{0}={1}/datacard_{2}_{3}.txt '.format(ch, boostcarddir, p, str(m)+'_'+w)
              cards_toy = cards_toy + '{0}={1}/datacard_{2}_{3}_toy.txt '.format(ch, boostcarddir, p, str(m)+'_'+w)
            elif 'leptonic' in ch:
              cards = cards + '{0}={1}/DatacardSignal_{2}.txt '.format(ch, lepcarddir, str(m))
              cards_toy = cards + '{0}={1}/DatacardSignal_{2}.txt '.format(ch, lepcarddir, str(m))
            elif 'resolve' in ch:
              cards = cards + '{0}={1}/RooParametricHistDataCard_M{2}_PseudoData_Combine.txt '.format(ch, resolvecarddir, str(m))
              cards_toy = cards + '{0}={1}/RooParametricHistDataCard_M{2}_PseudoData_Combine.txt '.format(ch, resolvecarddir, str(m))
...
#combine -M FitDiagnostics -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --cminFallbackAlgo Minuit,0.001 --rMin -1.0 --rMax 1.0 --robustFit=1 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000
combine -M FitDiagnostics -s 123456 -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH_toy.root --expectSignal 0 -t 1 --saveToys --setParameters QCD_tH_RA_norm=424,QCD_tH_RB_norm=58,QCD_tH_RC_norm=112,QCD_tH_RD_norm=15,QCD_tZ_RA_norm=5584.79,QCD_tZ_RB_norm=607.636,QCD_tZ_RC_norm=243.204,QCD_tZ_RD_norm=26.4611 --cminDefaultMinimizerTolerance=0.05  --cminFallbackAlgo Minuit,0.001 --rMin -1.0 --rMax 1.0 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000
combine -M AsymptoticLimits -s 123456 -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root -t 1 --toysFile higgsCombineTest.FitDiagnostics.mHMASS.123456.root --run expected -v 5 --rMin -0.1 --rMax 5 --cminDefaultMinimizerTolerance=0.2 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000 >& AsymptoticLimits_PROC_MASS.log
#combine -M AsymptoticLimits -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root -v 5 --rMin -0.1 --rMax 5 --cminDefaultMinimizerTolerance=0.2 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000 >& AsymptoticLimits_PROC_MASS.log
#mv higgsCombineTest.AsymptoticLimits.mHMASS.123456.root higgsCombineTest.AsymptoticLimits.mHMASS_WIDTH.123456.root

#combine -M MultiDimFit -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --algo grid --points 2000 --autoRange 4 --fastScan --autoBoundsPOIs "*" --saveNLL --saveFitResult --cminDefaultMinimizerTolerance=0.1 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000 >& MultiDimFit_PROC_MASS.log

#combine -s 654321 -M MultiDimFit -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --saveWorkspace --freezeParameters r --setParameters r=0 --cminDefaultMinimizerTolerance=0.2 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000 >& MultiDimFit_PROC_MASS_b.log
#combine -s 123456 -M MultiDimFit -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --saveWorkspace --cminDefaultMinimizerTolerance=0.2 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000 >& MultiDimFit_PROC_MASS_sb.log

#python nll_results_threaded.py MASS

#combineTool.py -M Impacts -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --rMin -10 --rMax 10 --doInitialFit --robustFit 1 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000 
#combineTool.py -M Impacts -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --rMin -10 --rMax 10 --doFits --robustFit 1 --parallel 10 --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000
#combineTool.py -M Impacts -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root --rMin -10 --rMax 10 -o impacts_PROC_MASS_WIDTH.json --setParameterRanges QCD_tH_RA_norm=0.0,100000:QCD_tH_RB_norm=0.0,100000:QCD_tH_RC_norm=0.0,100000:QCD_tZ_RA_norm=0.0,100000:QCD_tZ_RB_norm=0.0,100000:QCD_tZ_RC_norm=0.0,100000

#plotImpacts.py -i impacts_PROC_MASS_WIDTH.json -o impacts_PROC_MASS_WIDTH
 
PostFitShapesFromWorkspace -m MASS\
  -w ../../CARDDIR/datacard_PROC_MASS_WIDTH.root \
  -o postfitshapes_PROC_MASS_WIDTH.root \
  -f fitDiagnostics.root:fit_s \
  --postfit \
  --sampling \
  --print \
  >& postfit.log
...
combine -M FitDiagnostics -s 123456 -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH_toy.root --expectSignal 0 -t 1 --saveToys --cminDefaultMinimizerTolerance=0.2 --cminFallbackAlgo Minuit,0.001 --rMin -1.0 --rMax 1.0
combine -M AsymptoticLimits -m MASS -d ../../CARDDIR/datacard_PROC_MASS_WIDTH.root -t 1 --toysFile higgsCombineTest.FitDiagnostics.mHMASS.123456.root --run expected --expectSignal 0 -v 5 --rMin -0.1 --rMax 5 --cminPoiOnlyFit --cminDefaultMinimizerTolerance=0.2 >& AsymptoticLimits_PROC_MASS.log
mv higgsCombineTest.AsymptoticLimits.mHMASS.123456.root higgsCombineTest.AsymptoticLimits.mHMASS_WIDTH.123456.root
...
if __name__ == "__main__":
  main()
  #CombineAllCards('combined_cards', 'signal_JERUpdate', 'stephCards', 'svenjaCards')
